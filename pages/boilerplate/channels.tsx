...
export default function Channels() {
  ...
  useEffect(() => {
    ...
    ably.request("get", "/channels").then((res) => {
      console.log(res.items), setChannels(res.items);
    });
  }, []);

  return (
    <Layout pageTitle="View Channels" metaDescription="View Channels">
      <p className={homeStyles.description}>View the list of channels</p>

      <section className={styles.publish}>
        {channels.map((chan) => (
          <div key={chan.name}>{chan.name}</div>
        ))}
      </section>
